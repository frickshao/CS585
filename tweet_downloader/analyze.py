...
def to_text(tweets):
    '''Converts from a list of tweets to a list of tweet texts.
    Example Uses:
    tweet1 = {"text": "My hands are YUUGE!", ...}
    tweet2 = {"text": "#MAGA", ...}
    tweet3 = {"text": "I hate Little Marco.", ...}
    tweets = [tweet1, tweet2, tweet3]
    to_text(tweets) # => ["My hands are YUUGE!", "#MAGA", "I hate Little Marco."]
    '''
    return list(x['text'] for x in tweets)

def to_lowercase(tweets):
    '''Converts the text of each tweet in the list of tweets to lowercase.
    Example Uses:
    tweet1 = {"text": "My hands are YUUGE!", ...}
    tweet2 = {"text": "#MAGA", ...}
    tweet3 = {"text": "I hate Little Marco.", ...}
...
def lowercase_helper(tweet):
    return {**tweet,**{'text': tweet['text'].lower()}}

def nonempty(tweets):
    '''Removes all tweets with empty texts from the list of tweets.
    Example Uses:
    tweet1 = {"text": "#MAGA", ...}
    tweet2 = {"text": "", ...}
    tweet3 = {"text": "#CrookedHillary", ...}
    tweets = [tweet1, tweet2, tweet3]
    ...
    return list(filter(lambda tweet: tweet['text'] != '', deepcopy(tweets)))

def total_word_count(tweets):
    ''' Calculates the total number of words in each tweet in a list.
    Example Uses:
    tweet1 = {"text": "I am President now.", ...}
    tweet2 = {"text": "Make America Safe Again!", ...}
    tweet3 = {"text": "Make America Great Again!", ...}
    tweets = [tweet1, tweet2, tweet3]
...
def hashtags(tweet):
    ...
    Example Uses:
    tweet = {"text": "Hello, America. #MAGA #Trump2016", ...}
    hashtags(tweet) # => ["#MAGA", "#Trump2016"]
    '''
    return list(filter(lambda x: '#' in x, tweet['text'].split()))

def mentions(tweet):
    '''– Gets a list of all the mentions from the specified tweet.
    Example Uses:
    tweet = {"text": "@aatxe would be a better president than me.", ...}
...
def all_hashtags(tweets):
    ...
    Example Uses:
    tweet1 = {"text": "Hello, America. #MAGA #Trump2016", ...}
    tweet2 = {"text": "Lock her up! #CrookedHillary #MAGA", ...}
    tweet3 = {"text": "No hashtags in here.", ...}
    tweets = [tweet1, tweet2, tweet3]
...
def all_mentions(tweets):
    ...
    Example Uses:
    tweet1 = {"text": "@cnn is fake news!", ...}
    tweet2 = {"text": "Can’t believe how many people buy @cnn fake news!", ...}
    tweet3 = {"text": "@marcorubio and I are friends now!", ...}
    tweets = [tweet1, tweet2, tweet3]
...
def count_individual_hashtags(tweets):
    '''tweet1 = {"text": "#MAGA #Trump2016", ...}
    tweet2 = {"text": "#MakeAmericaGreatAgain", ...}
    tweet3 = {"text": "No hashtags in here.", ...}
    tweets = [tweet1, tweet2, tweet3]
...
def iphone_tweets(tweets):
    '''Filters a list of tweets to find only those made from an iPhone.
        Example Uses:
       tweet1 = {"text": "My hands are YUUGE!", "source": "Twitter for iPhone", ...}
        tweet2 = {"text": "#MakeAmericaOkAgain", "source": "Twitter for Android", ...}
        tweet3 = {"text": "#CrookedHillary", "source": "Twitter for iPhone", ...}
        tweets = [tweet1, tweet2, tweet3]
...
def android_tweets(tweets):
    ...
    tweet1 = {"text": "My hands are YUUGE!", "source": "Twitter for iPhone", ...}
    tweet2 = {"text": "#MakeAmericaOkAgain", "source": "Twitter for Android", ...}
    tweet3 = {"text": "I hate Little Marco.", "source": "Twitter for iPhone", ...}
    tweets = [tweet1, tweet2, tweet3]
